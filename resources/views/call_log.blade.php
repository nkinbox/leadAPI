            ...
            <div v-if="phone_number" class="position-absolute search-result p-3">
                <div class="p-2">
                        ...
                        <div v-else>
                            ...
                            <div v-else="selected_agent.user_name" class="alert alert-danger text-center h5 p-3">
                                No Record Found
                            </div>
                        </div>
                </div>
                        ...
                        <table class="table table-sm">
                            ...
                            <tbody class="text-white">
                                <tr v-for="(log, index) in filtered_call_logs" :key="index" :class="{'bg-danger':(log.status?false:true), 'bg-success':(log.status?true:false)}">
                                    <th scope="row" v-text="index+1"></th>
                                    <td class="small" v-html="log.agent_name+'<br>'+log.agent_phone_number"></td>
                                    <td class="small pointer" v-html="(log.saved_name?log.saved_name:'NA')+'<br>'+log.dial_code+' '+log.phone_number" @click="searchPhoneNumber(log.phone_number)"></td>
                                    <td>
                                        <div v-text="log.call_type" class="badge font-weight-normal" :class="{'badge-info':(log.call_type == 'outgoing'), 'badge-warning':(log.call_type == 'incoming' || log.call_type == 'missed')}"></div>
                                    </td>
                                    <td v-text="seconds(log.duration)"></td>
        ...
        var app = new Vue({
            ...
            methods: {
                searchPhoneNumber(phone_number) {
                    search.phone_number = phone_number
                    search.fetchLogs()
                },
                setFilter(type = 'all', status = 'all', unique = false) {
                    this.filter = {
