...
class CallRecorderController extends Controller {
    ...
    public function displayLog(Request $request) {
        ...
        DB::statement('create temperory table temp_unique_calls select min(id) as id, group_concat(distinct call_type), count(id) as count_number from temp_call_logs group by concat(dial_code, phone_number, call_type)');
        DB::update('update temp_call_logs inner join temp_unique_calls on temp_unique_calls.id = temp_call_logs.id set is_unique = 1');

        dump(DB::table('temp_unique_calls')->get());
